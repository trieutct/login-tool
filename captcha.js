...
function extractColor(text) {
    ...
    return null; // Không tìm thấy màu nào
}
    ...
    try {
        ...
        image.scan(0, 0, image.bitmap.width, image.bitmap.height, function (x, y, idx) {
            ...
            if (
                ...
                this.bitmap.data[idx + 3] = 0; // làm trong suốt pixel khác màu
            }
        });

        // Lưu tạm ra file (nếu cần)
        const filteredImagePath = `${app_id}_filtered_fa.png`;
        await image.writeAsync(filteredImagePath);

        // Tạo chuỗi Base64 ảnh mới
        const filteredBuffer = await image.getBufferAsync(Jimp.MIME_PNG);
        const filteredBase64 = `data:image/png;base64,${filteredBuffer.toString('base64')}`;

        console.log(`✅ Ảnh đã lưu: ${filteredImagePath}`);
